...
>
> **后续演进（LLM 方案 L1–L5b，见 [`llm_inference_plan.md`](llm_inference_plan.md)）**：本文的描述符格式与取指单元是
> LLM 推理的基础，之后又扩展过：
> - **L1 命令处理扩展**（§5.3），CAPS bit 24：
>   - BASE0–15；
>   - PARAM0–7；
>   - w0[31:16] 是两个动态槽，原为保留位；
>   - 新 opcode：0x13 LOOP_END、0x14 SETREG、0x15 CALL、0x16 RET、0x17 LDPARAM；
>   - JUMP 可以用相对地址。
> - **主机通知**用 `mat_notify` 与 `notify_irq`（CAPS bit 22）实现。END 的 IRQ 位**仍未实现**：
>   ARM 访问不到加速器的 CSR，所以通知做成边沿中断，由常驻固件 `firmware/rt` 在命令环条目完成时发出。
> - **L2 的 VE 描述符**：用到 w3[63:53]、w5[63:32]、w6、w7；types[5:4] 是 src2 的类型；op 6 是 TRANSPOSE（CAPS bit 23）。
> - **计数器没有增加事件**：L2 的 fp32 VE 与整数 VE 共用现有的 VE 事件，按位或。
> - 当前的 bitstream 是 `bitstreams/l2`（D=8，44,005 LUT）。
>   stories15M 的整张列表每个 token 执行 2,477 条描述符（L4）；8 条序列批处理时每步执行 7,231 条（L5b）。
...
**公共头 `w0`**（本节是 M5 的格式；L1、L2 的扩展见文首的“后续演进”和 `llm_inference_plan.md` §5.3、§6.6、附录 A）
